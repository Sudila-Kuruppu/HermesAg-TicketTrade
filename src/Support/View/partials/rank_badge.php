 ...
 *
 * Phase 6 Plan 06-01 ADDS the canonical tooltip format per EXPERIENCE.md
 * L143 / 06-UI-SPEC.md Copywriting Contract:
 *   "{name} ({code}) — {min} to {max} points" for E..A
 *   "{name} ({code}) — {max}+ points" for S
 * The threshold numbers come from config/ranks.php (single source of
 * truth) — the partial does NOT duplicate the ladder.
 *
...
// Canonical tooltip text — derived from config/ranks.php (per-tier
// min + next-tier min). Single source of truth.
$tierOrder = array_keys($ladder); // ['E','D','C','B','A','S']
$tierIdx = array_search($tier, $tierOrder, true);
$nextMin = isset($tierOrder[$tierIdx + 1])
    ? (int) $ladder[$tierOrder[$tierIdx + 1]]['min_points']
    : null;
if ($nextMin === null) {
    // Top tier (S): "{name} ({code}) — {max}+ points".
    $tooltipText = sprintf('%s (%s) — %d+ points', $def['name'], $tier, (int) $def['min_points']);
} else {
    // E..A: "{name} ({code}) — {min} to {max} points".
    $tooltipText = sprintf(
        '%s (%s) — %d to %d points',
        $def['name'],
        $tier,
...
if ($tier === 'S') : ?>
  <span class="rank-badge rank-badge--S legend-glow" data-tier="S" data-bs-toggle="tooltip" title="<?= htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') ?>">
...
<?php elseif ($tier === 'A') : ?>
  <span class="rank-badge rank-badge--A" data-tier="A" data-bs-toggle="tooltip" title="<?= htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') ?>">
    <svg viewBox="0 0 32 32" width="<?= $size ?>" height="<?= $size ?>" aria-hidden="true">
...
<?php elseif ($tier === 'B') : ?>
  <span class="rank-badge rank-badge--B" data-tier="B" data-bs-toggle="tooltip" title="<?= htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') ?>">
    <svg viewBox="0 0 32 32" width="<?= $size ?>" height="<?= $size ?>" aria-hidden="true">
...
<?php elseif ($tier === 'C') : ?>
  <span class="rank-badge rank-badge--C" data-tier="C" data-bs-toggle="tooltip" title="<?= htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') ?>">
    <svg viewBox="0 0 32 32" width="<?= $size ?>" height="<?= $size ?>" aria-hidden="true">
...
<?php elseif ($tier === 'D') : ?>
  <span class="rank-badge rank-badge--D" data-tier="D" data-bs-toggle="tooltip" title="<?= htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') ?>">
    <svg viewBox="0 0 32 32" width="<?= $size ?>" height="<?= $size ?>" aria-hidden="true">
...
<?php else : // E (Recruit) ?>
  <span class="rank-badge rank-badge--E" data-tier="E" data-bs-toggle="tooltip" title="<?= htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') ?>">
    <svg viewBox="0 0 32 32" width="<?= $size ?>" height="<?= $size ?>" aria-hidden="true">
